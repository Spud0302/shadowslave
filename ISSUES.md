...
| 2.12 | **Losing an Aspect leaves its attribute modifier applied forever.** CONFIRMED in-game v1.2.0: removing `ss_aspect_bone` and adding `ss_aspect_wind` left the player with 6 armour still applied — the upkeep stopped reapplying it, but nothing removed the modifier already on them. `awaken/roll` also never clears previous Aspect/Flaw tags, so a re-roll compounds it. Latent in normal play (you Awaken once), but any re-roll leaves you buffed by an Aspect you no longer have. | Clear all eight `ss_aspect_*` / `ss_flaw_*` tags **and** strip the four attribute modifiers at the top of `roll.mcfunction`, before applying the new roll. |
