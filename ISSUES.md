...
| 3.5 | **Getting out of bed may BE the entry gesture.** Crouch is how you leave a bed, and crouching on a bed is how a Carrier enters the trial — so a freshly marked Carrier lying in a bed may have no way to stand up that does not drag them in. **Predicted, NOT confirmed** — the one time it looked like this had happened, the crouch was deliberate. Needs a specific test: as a Carrier, sleep through a night, then try to get out of bed normally. | If it reproduces: arm the sneak entry on a short delay — `infect` (and waking) sets a countdown, the tick loop decrements it, and the sneak check requires it absent or zero. The player gets up, reads the message, and enters when they choose to. |
