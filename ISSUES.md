...
| 3.5 | **Getting out of bed IS the entry gesture.** Crouch is how you leave a bed, and crouching on a bed is how a Carrier enters the trial — so the moment the first sleep marks you, you are lying in a bed with no way to stand up that does not drag you into the nightmare. Confirmed in-game v1.3.0. The deliberate-hold requirement exists precisely so entry is never accidental, and this defeats it. | Arm the sneak entry on a short delay: `infect` sets a countdown (say 60 ticks), the tick loop decrements it, and the sneak check requires it to be absent or zero. The player gets out of bed, reads the message, and enters when they choose to. |
