...
| 1.6 | **Dying in the nightmare loses your items — INTERMITTENT, not fixed.** v1.4.2's scheduled sweep moved the drops home in one harness run (a diamond arrived at the return point) but not in repeat runs, where the dropped diamonds are found in **neither** dimension afterwards — so they are not being mis-placed, they are not surviving. Two earlier approaches failed outright: tagging drops during teardown matched nothing (Minecraft spawns them after the tick health hits 0), and a marker as teleport destination never resolved (selectors are dimension-scoped). | Unresolved. Next step is to establish whether the drops exist at all a few ticks after death — if they never spawn, the sweep is chasing something that is not there and the fix belongs at the death path, not the sweep. |
