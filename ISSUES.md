...
| 1.7 | **Entering a nightmare right after `/trigger soul` ejects you instantly.** `soul.mcfunction:26` reuses the `ss_health` objective as scratch to hold the player's *armour* for the readout — a shortcut I marked with a `ponytail:` comment claiming it was always overwritten before a real read. It is not. On the arrival tick of a cross-dimension teleport the player entity is mid-rebuild, so `tick_player:6`'s `data get entity @s Health` fails and leaves the score at its previous value; `tick_player:7` then reads the armour number against the `..8` ejection threshold. With 6 armour that is an instant ejection. Confirmed in-game v1.2.0 — and it **persists**: every subsequent entry ejects too, because nothing refreshes the stale score (the ejection path does not write it, and each new arrival tick fails the read again). The mod becomes unenterable until something else happens to update `ss_health`. | Give the soul readout its own scratch objective (`ss_scratch`) instead of borrowing `ss_health`. Belt and braces: gate the ejection on `matches 1..8` so a failed or zero read cannot trigger it. |
