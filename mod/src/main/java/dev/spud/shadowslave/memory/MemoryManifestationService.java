...
public final class MemoryManifestationService {
    ...
    public static void clearAshCompassManifestations(ServerPlayer player) {
        remove(Objects.requireNonNull(player, "player").getInventory(), ModItems.ASH_COMPASS_MEMORY.get());
    }

    public static void clearGlassRoadManifestations(ServerPlayer player) {
        remove(Objects.requireNonNull(player, "player").getInventory(), ModItems.GLASS_ROAD_MEMORY.get());
    }
    ...
    private static boolean has(Inventory inventory, Item item) {
