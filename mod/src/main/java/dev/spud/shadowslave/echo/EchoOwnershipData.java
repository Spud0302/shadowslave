...
public record EchoOwnershipData(List<EchoInstanceData> echoes) {
    private static final MapCodec<EchoOwnershipData> RAW_CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
            EchoInstanceData.CODEC.codec().listOf().optionalFieldOf("echoes", List.of()).forGetter(EchoOwnershipData::echoes)
    ).apply(instance, EchoOwnershipData::new));
    public static final MapCodec<EchoOwnershipData> CODEC = RAW_CODEC.flatXmap(value -> {
        try { return DataResult.success(new EchoOwnershipData(value.echoes())); }
        catch (IllegalArgumentException | NullPointerException exception) { return DataResult.error(() -> "Invalid EchoOwnershipData: " + exception.getMessage()); }
    }, DataResult::success);
    ...
    public EchoOwnershipData {
        ...
        for (EchoInstanceData echo : echoes) {
            ...
            if (!ids.add(echo.echoId())) throw new IllegalArgumentException("Duplicate owned Echo id: " + echo.echoId());
        }
    }
    public static EchoOwnershipData empty() { return new EchoOwnershipData(List.of()); }
    public Optional<EchoInstanceData> find(ResourceLocation echoId) { return echoes.stream().filter(echo -> echo.echoId().equals(Objects.requireNonNull(echoId))).findFirst(); }
    public boolean owns(ResourceLocation echoId) { return find(echoId).isPresent(); }
    public EchoOwnershipData award(EchoInstanceData echo) {
        ...
        if (existing.isPresent()) {
            ...
            if (!owned.formalName().equals(echo.formalName()) || !owned.acquisitionSource().equals(echo.acquisitionSource()) || !owned.provenance().equals(echo.provenance()))
    ...
    public EchoOwnershipData withCargo(ResourceLocation echoId, ResourceLocation itemId, int count) { return replaceEcho(echoId, echo -> echo.withCargo(itemId, count)); }
    public EchoOwnershipData withoutCargo(ResourceLocation echoId) { return replaceEcho(echoId, EchoInstanceData::withoutCargo); }
    ...
    private EchoOwnershipData replaceEcho(ResourceLocation echoId, java.util.function.UnaryOperator<EchoInstanceData> update) {
        Objects.requireNonNull(echoId); Objects.requireNonNull(update);
        ArrayList<EchoInstanceData> next = new ArrayList<>(echoes.size()); boolean found = false; boolean changed = false;
        for (EchoInstanceData echo : echoes) {
            if (echo.echoId().equals(echoId)) { found = true; EchoInstanceData updated = Objects.requireNonNull(update.apply(echo)); next.add(updated); changed |= updated != echo; }
            else next.add(echo);
        }
        if (!found) throw new IllegalArgumentException("Cannot update unowned Echo: " + echoId);
