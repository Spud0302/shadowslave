...
public record EchoInstanceData(ResourceLocation echoId, String formalName, String acquisitionSource, String provenance,
    ...
    private static final Codec<EchoContentCatalog.CommandMode> COMMAND_MODE_CODEC = Codec.STRING.flatXmap(
            value -> {
                try { return DataResult.success(EchoContentCatalog.CommandMode.valueOf(value.trim().toUpperCase(Locale.ROOT))); }
                catch (IllegalArgumentException | NullPointerException exception) { return DataResult.error(() -> "Unknown Echo command mode: " + value); }
    ...
    public EchoInstanceData {
        ...
        commandTargetDimension = Objects.requireNonNull(commandTargetDimension, "commandTargetDimension");
        commandTargetBlockPos = Objects.requireNonNull(commandTargetBlockPos, "commandTargetBlockPos");
        if (commandTargetDimension.isPresent() != commandTargetBlockPos.isPresent()) throw new IllegalArgumentException("Echo command target dimension and block position must be stored together");
        cargoItemId = Objects.requireNonNull(cargoItemId, "cargoItemId");
        cargoCount = Objects.requireNonNull(cargoCount, "cargoCount");
        if (cargoItemId.isPresent() != cargoCount.isPresent()) throw new IllegalArgumentException("Echo cargo item id and count must be stored together");
        cargoCount.ifPresent(count -> { if (count <= 0 || count > 99) throw new IllegalArgumentException("Echo cargo count must be between 1 and 99"); });
        ...
        int populated = (manifestationEntityUuid.isPresent() ? 1 : 0) + (manifestationDimension.isPresent() ? 1 : 0) + (manifestationBlockPos.isPresent() ? 1 : 0);
        if (populated != 0 && populated != 3) throw new IllegalArgumentException("Echo manifestation UUID, dimension and block position must be stored together");
    }
    ...
    public EchoInstanceData withCommandMode(EchoContentCatalog.CommandMode mode) {
        ...
        Optional<ResourceLocation> nextDimension = checked == EchoContentCatalog.CommandMode.GUARD_POINT ? commandTargetDimension : Optional.empty();
    ...
    public EchoInstanceData withGuardPoint(ResourceLocation dimension, BlockPos position) {
        ...
        long checkedPosition = Objects.requireNonNull(position, "position").asLong();
        if (commandMode == EchoContentCatalog.CommandMode.GUARD_POINT && commandTargetDimension.equals(Optional.of(checkedDimension)) && commandTargetBlockPos.equals(Optional.of(checkedPosition))) return this;
        return new EchoInstanceData(echoId, formalName, acquisitionSource, provenance, EchoContentCatalog.CommandMode.GUARD_POINT,
                Optional.of(checkedDimension), Optional.of(checkedPosition), cargoItemId, cargoCount, manifestationEntityUuid, manifestationDimension, manifestationBlockPos);
    }
    public EchoInstanceData withCargo(ResourceLocation itemId, int count) {
        ...
        return new EchoInstanceData(echoId, formalName, acquisitionSource, provenance, EchoContentCatalog.CommandMode.CARRY,
                Optional.empty(), Optional.empty(), Optional.of(Objects.requireNonNull(itemId, "itemId")), Optional.of(count), manifestationEntityUuid, manifestationDimension, manifestationBlockPos);
    }
    public EchoInstanceData withoutCargo() {
        if (cargoItemId.isEmpty()) return this;
        EchoContentCatalog.CommandMode nextMode = commandMode == EchoContentCatalog.CommandMode.CARRY ? EchoContentCatalog.CommandMode.HOLD : commandMode;
        return new EchoInstanceData(echoId, formalName, acquisitionSource, provenance, nextMode, commandTargetDimension, commandTargetBlockPos,
                Optional.empty(), Optional.empty(), manifestationEntityUuid, manifestationDimension, manifestationBlockPos);
    }
    ...
    public EchoInstanceData withManifestation(UUID entityUuid, ResourceLocation dimension, BlockPos position) {
        return new EchoInstanceData(echoId, formalName, acquisitionSource, provenance, commandMode, commandTargetDimension, commandTargetBlockPos,
                cargoItemId, cargoCount, Optional.of(Objects.requireNonNull(entityUuid).toString()), Optional.of(Objects.requireNonNull(dimension)), Optional.of(Objects.requireNonNull(position).asLong()));
    }
    public EchoInstanceData withoutManifestation() {
        if (manifestationEntityUuid.isEmpty()) return this;
        return new EchoInstanceData(echoId, formalName, acquisitionSource, provenance, commandMode, commandTargetDimension, commandTargetBlockPos,
                cargoItemId, cargoCount, Optional.empty(), Optional.empty(), Optional.empty());
    }
