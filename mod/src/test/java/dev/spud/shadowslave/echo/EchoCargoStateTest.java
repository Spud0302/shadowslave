...
class EchoCargoStateTest {
    ...
    @Test void cargoRoundTripPersistsExactPlainStackIdentityAndCount() {
        EchoOwnershipData carrying = EchoOwnershipData.empty().award(unmanifested()).withCargo(ASH_BURROWER, COBBLESTONE, 32);
    ...
    private static EchoInstanceData unmanifested() {
        return new EchoInstanceData(ASH_BURROWER, "Ash Burrower", "test_award", "test", Optional.empty(), Optional.empty(), Optional.empty());
    }
