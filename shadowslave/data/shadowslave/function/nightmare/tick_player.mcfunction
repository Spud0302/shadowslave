...
execute if score @s ss_health matches ..11 run function shadowslave:nightmare/eject
...
execute if entity @s[tag=ss_creature_spawned] store result bossbar shadowslave:trial value run data get entity @e[tag=ss_creature,limit=1,distance=..128] Health

# Creature was summoned and is now gone: the trial is won.
execute if entity @s[tag=ss_creature_spawned] unless entity @e[tag=ss_creature,limit=1,distance=..128] run function shadowslave:nightmare/survive
