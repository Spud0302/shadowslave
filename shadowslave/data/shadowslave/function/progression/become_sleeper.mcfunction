...
#
# Keep progression state separate from the prototype Aspect/Flaw generator. The old
# awaken/roll.mcfunction mixed those responsibilities together, which made the incorrect
# First Nightmare -> Awakened model harder to remove later.

scoreboard players set @s ss_rank 1

# PROTOTYPE-LIMIT: these four Aspects and four Flaws are temporary. Canon Aspects are unique
# and Flaws are personal rather than an independent random roll. Keeping the placeholder
# generator behind one call gives the eventual Java/data-driven system a single seam to replace.
