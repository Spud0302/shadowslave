...
Production opens `world/data/shadowslave_nightmares.dat` and requires a structurally readable `data.instances` list containing neither:

- the dead player's UUID; nor
- the dead Nightmare instance UUID.

Malformed active-instance entries, a missing registry file, a missing `data` compound, or a missing `instances` list fail closed while the already-durable death marker remains available for retry/reconstruction.

Exact persisted-state verification is deliberate instead of a changed-byte digest. On a restart after ownership removal was already persisted, the target registry image can already be correct and need no further byte change. The relevant proof is that the persisted ownership authority is absent before the separate death authority is consumed.
...
- **INFERRED:** none added.
- **DESIGN:** canonical-death recovery authority in one SavedData surface must not be consumed until the other persisted authority surface proves active ownership is gone.
...
`NightmareDeathCoordinatorTest` now proves that failure of the ownership-teardown persistence proof retains the death marker and prevents its deletion, while the existing restart-at-every-durable-boundary matrix remains.
